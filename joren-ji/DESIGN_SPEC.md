...
LINEのリッチメニューから遷移する静蓮寺のWebページ群。LINE内ブラウザ（WebView）での表示を前提とする。
...
│   └── flow.html           ← 当日の流れ
└── images/
    ├── jorenji_hondo.jpg       ← ヒーロー画像（住職後ろ姿・法要）
    ├── jorenji_priest.jpg      ← 住職写真
    ├── jorenji_carousel_1.jpg  ← カルーセル1
    ├── jorenji_carousel_2.jpg  ← カルーセル2（= jorenji_hondo.jpg のソース）
    └── jorenji_carousel_3.jpg  ← カルーセル3
```
...
**GoogleマップURL：**
```
https://maps.google.com/maps?q=静蓮寺+東京都台東区下谷1-12-21&output=embed
```
